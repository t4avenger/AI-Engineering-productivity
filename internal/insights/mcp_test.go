...
func TestMCPInventoryFromEventsReportsUsageAndHeuristicTokenContext(t *testing.T) {
	events := []canonical.Event{
		...
		testMCPEvent("connected", "mcp_server_connection", map[string]any{}, map[string]any{"event": map[string]any{"server_fingerprint": "mcp:hmac:filesystem", "status": "connected", "server_scope": "user", "transport_type": "stdio", "is_plugin": false}}),
	...
	if !byFingerprint["mcp:hmac:filesystem"].Used || byFingerprint["mcp:hmac:filesystem"].ContextWasteState != "used" {
...
func TestMCPInventoryDoesNotExposeRawServerIdentity(t *testing.T) {
	inventory := MCPInventoryFromEvents([]canonical.Event{
		testMCPEvent("connected", "mcp_server_connection", map[string]any{}, map[string]any{"event": map[string]any{"server_name": "raw-secret-filesystem", "status": "connected"}}),
	})
	payload, err := json.Marshal(inventory)
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(payload), "raw-secret-filesystem") {
		t.Fatalf("raw MCP server identity leaked: %s", payload)
	}
}

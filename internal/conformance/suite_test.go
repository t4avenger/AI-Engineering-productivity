	...
	for _, a := range adapters() {
		...
		t.Run(a.name, func(t *testing.T) {
			...
			for i, record := range records {
				...
				hasTokens := record.InputTokens != nil || record.OutputTokens != nil
				if record.Model != "" && hasTokens {
			...
			if a.profile.modelIdentity.promotable() && a.profile.inputOutputTokens.promotable() && observed == 0 {

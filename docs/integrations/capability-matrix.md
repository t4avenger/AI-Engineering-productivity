...
| Skill invocations | unknown | unknown | unknown | Mark `explicit | inferred | unavailable` only after event fixture evidence. Claude Code: no skill signal in the committed fixture, so `claude.NormalizeEvents`/`ExtractModelInteractions` stamp `provider_extensions.skill_detection = "unavailable"`. |

...
| Tool execution | partial | Current logs expose `codex.tool_decision`, `codex.tool_result`, and `codex.sandbox_outcome`; current metrics expose `codex.tool.call`, `codex.tool.call.duration_ms`, `codex.tool.unified_exec`, and `codex.turn.tool.call`. Canonical tool-call mapping remains follow-up work (#113). |

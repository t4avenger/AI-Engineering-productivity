...
| Tool execution | partial | `codex.tool_result` logs now map to first-class tool-call signals and `canonical.Operation` records with evidence from `fixtures/codex/observed-sanitised/codex-0.153.4-outcome-contracts-otlp.json` and golden output `fixtures/codex/expected/codex-0.153.4-tool-result.operations.json`; `codex.sandbox_outcome` logs now map to command-execution operation signals with evidence from `fixtures/codex/observed-sanitised/codex-0.153.4-sandbox-outcome-otlp.json` and golden output `fixtures/codex/expected/codex-0.153.4-sandbox-outcome.operations.json`; `codex.tool_decision` and metric-only tool counters remain follow-up surfaces. |

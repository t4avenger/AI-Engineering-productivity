...
| Session identifier | partial | `conversation.id` is retained in the local-only edition as `codex:<conversation.id>` when present; `request_id` and fallback session identity still use installation HMAC fingerprints. |

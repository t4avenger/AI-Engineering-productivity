...
then revalidated with CLI 0.154.0 on 2026-09-17 after the separate `otel.trace_exporter` appeared, and with CLI 0.155.1 on 2026-09-20 for trace-to-log conversation identity. Both captures used a temporary `CODEX_HOME`,
...
| OTLP trace export | version-dependent | CLI 0.153.4 emitted no `/v1/traces` POSTs; CLI 0.154.0 emitted both OTLP/HTTP JSON and binary protobuf from `codex_exec`, and PTY-backed `codex_cli_rs` flushed binary batches on shutdown. CLI 0.155.1 additionally emits resource-level `conversation.id`; the sanitised same-run trace/log fixtures prove it exactly matches the log value. Evidence: `fixtures/codex/observed-sanitised/codex-0.154.0-trace-spans-otlp.json`, `codex-0.155.1-trace-conversation-otlp.json`, and `codex-0.155.1-log-conversation-otlp.json`. |

...
then revalidated with CLI 0.154.0 on 2026-09-17 after the separate `otel.trace_exporter` appeared, and with CLI 0.155.1 on 2026-09-20 and 2026-09-26 for trace-to-log identity and turn correlation. Both captures used a temporary `CODEX_HOME`,
...
| OTLP trace export | version-dependent | CLI 0.153.4 emitted no `/v1/traces` POSTs; CLI 0.154.0 emitted both OTLP/HTTP JSON and binary protobuf from `codex_exec`, and PTY-backed `codex_cli_rs` flushed binary batches on shutdown. CLI 0.155.1 additionally emits resource-level `conversation.id`; a separate 0.155.1 `session_task.turn` fixture emits `thread.id` and `turn.id`. Same-run sanitised trace/log fixtures prove the exact provider-key joins. Evidence: `fixtures/codex/observed-sanitised/codex-0.154.0-trace-spans-otlp.json`, `codex-0.155.1-trace-conversation-otlp.json`, `codex-0.155.1-log-conversation-otlp.json`, `codex-0.155.1-trace-thread-turn-otlp.json`, and `codex-0.155.1-log-thread-turn-otlp.json`. |
...
| Approval / permission decisions | supported | `codex.tool_decision` logs map to event-level approval signals with evidence from `fixtures/codex/observed-sanitised/codex-0.153.4-tool-decision-otlp.json` and golden output `fixtures/codex/expected/codex-0.153.4-tool-decision.events.json`; account identifiers, emails, hostnames, credentials, prompt/response fields, and slug values remain unretained. Provider-emitted command/argument/output fields remain local raw tool evidence. |
| Pull-request / merge-request URL | supported | Codex 0.155.1 `codex.tool_result` retains a URL in `arguments`; `fixtures/codex/observed-sanitised/codex-0.155.1-tool-pr-link-otlp.json` proves promotion to `attributes.pr_link` only when one distinct recognised HTTP(S) pull/merge-request URL exists in a provider session. GitHub, GitLab, Bitbucket, Azure DevOps, and compatible self-hosted path forms are accepted verbatim; conflicts stay `partial`, and no URL is inferred from branch, counts, repository metadata, or a network lookup. |

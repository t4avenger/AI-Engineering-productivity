...
| `entrypoint` | Entrypoint | Normalized tool entrypoint retained from provider evidence, for example Codex `interactive` / `codex exec` (from service metadata, #119) or Claude Code transcript `cli` (from `provider_extensions.transcript.entrypoint`, #91 / #158). | Label | Show unavailable when no reviewed event carries entrypoint evidence; do not infer from session ID alone. OTLP `app.entrypoint` is still owned by #107. |
| `git_branch` | Branch | Git branch retained from Claude Code session JSONL transcript envelope (`provider_extensions.transcript.git_branch`) and promoted onto the session (#158). | Identifier | Show unavailable when absent; never invent from repository heuristics. |
| `pr_link` | PR | Pull-request or merge-request URL retained raw when one distinct reviewed candidate is observed for the session. | URL | Show observed for one distinct retained URL, `partial` for conflicting retained URLs, and unavailable when none is retained; do not invent from `pull_request.count` or similar counters. |
...
| Session governance checklist | Governance checklist | Session-scoped risky-access and unapproved-MCP outcomes evaluated over all retained events for the selected session, independent of timeline pagination. | Outcome, visibility, finding count | Preserve `indeterminate`, `policy_unconfigured`, and event-read failure as unavailable; never infer a clean result. Raw evidence remains on `/governance`. |
| `total_operations` | Total operations | Count of persisted canonical operation records. | Count | Count only retained operation records; do not infer from session counts. |
| `average_duration_ms` | Average duration | Mean duration across operation records with observed `duration_ms`. | Duration | Average only observed duration samples; unavailable durations are excluded, not treated as zero. |
